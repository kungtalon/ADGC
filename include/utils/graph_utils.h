 ...
 public:
  ...
  inline Agnode_t *add_node(std::string_view node_name, const std::string &node_type) {
    ...
    if (node_type == "variable") {
      set_node_attr(node, "label", node_name);
      set_node_attr(node, "fillcolor", auto_diff::GraphViz::VAR_GRAPHVIZ_NODE_COLOR);
      ...
      set_node_attr(node, "fillcolor", auto_diff::GraphViz::OPS_GRAPHVIZ_NODE_COLOR);
    } else if (node_type.rfind("F") == 0) {
      // type starts with F, functional
      set_node_attr(node, "label", node_name.substr(2, node_name.size() - 2));
      set_node_attr(node, "fillcolor", auto_diff::GraphViz::FUNC_GRAPHVIZ_NODE_COLOR);
    } else {
      set_node_attr(node, "fillcolor", auto_diff::GraphViz::OTHER_GRAPHVIZ_NODE_COLOR);
    }
    return node;
  }
